...
import React, { useState, useEffect, useCallback, useMemo } from 'react'; // Додайте useMemo
...
const HistoryPage: React.FC = () => {
    ...
    // Оголошуємо queryParams тут, щоб він був доступний
    const queryParams = useMemo(() => new URLSearchParams(location.search), [location.search]);

    // Тепер initialFilterType може безпечно використовувати queryParams
    ...
    // Отримання унікальних типів для фільтра
    // Використовуємо useCallback, щоб уникнути непотрібного переобчислення
    const uniqueTypes = React.useMemo(() => {
        return ['All', ...new Set(mockHistoryData.map(entry => entry.type))];
    }, []); // Залежності від mockHistoryData (якщо вона може змінюватися, що малоймовірно для моків)

    // useCallback для функції фільтрації та сортування
    const applyFiltersAndSort = useCallback(() => {
        let currentHistory = [...mockHistoryData];

        // 1. Фільтрація за типом (з сайдбару або селекту)
        if (filterType !== 'All') {
    ...
    }, [searchTerm, filterType, sortBy]); // Залежності для useCallback

    // Ефект для застосування фільтрів та сортування при зміні відповідних станів
    useEffect(() => {
        applyFiltersAndSort();
    }, [applyFiltersAndSort]);

    // Ефект для оновлення фільтра типу при зміні URL (наприклад, з кліка сайдбару)
    useEffect(() => {
    ...
    const handleViewDetails = (entry: HistoryEntry) => {
        // Навігація до сторінки деталей за ID запису
    ...
    const formatTimestamp = (isoString: string) => {
        ...
        return date.toLocaleString(); // Форматування дати та часу
    };

    return (
        <div className="bg-light-bg dark:bg-dark-bg p-4 md:p-8 min-h-screen"> {/* Додано min-h-screen */}
            <h1 className="text-3xl font-bold text-light-text dark:text-dark-text mb-6">History</h1>

            {/* Панель фільтрів та пошуку */}
            <div className="flex flex-col md:flex-row gap-4 mb-8 p-4 bg-light-surface dark:bg-dark-surface rounded-lg shadow-md border border-light-border dark:border-dark-border">
                    ...
                    filteredHistory.map((entry) => (
                        ...
                        >
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center space-x-2 flex-wrap"> {/* Додано flex-wrap для адаптивності бейджів */}
                                    {/* `capitalize` додано для автоматичного перетворення тексту бейджа на заголовок, якщо потрібно */}
                                    <Badge variant="default" className="capitalize mb-1 md:mb-0">{entry.modelUsed}</Badge>
                                    <Badge variant={getStatusBadgeVariant(entry.status)} className="mb-1 md:mb-0">{entry.status}</Badge>
                                    {entry.statusReason && (
                                        <span className="text-sm text-light-error dark:text-dark-error ml-2 mt-1 md:mt-0">({entry.statusReason})</span>
                                    )}
                                </div>
                            </div>
                            <div className="flex flex-wrap gap-2 md:flex-none mt-4 md:mt-0"> {/* Адаптивність для кнопок */}
                                <Button
                                ...
                                {/* Download & Delete Buttons (placeholder functionality) */}
                                <Button
                                ...
                                <Button
                                    onClick={() => { if (window.confirm('Are you sure you want to delete this entry?')) alert(`Deleting: ${entry.id}`) }}
                                    variant="danger"
                                    size="small"
                                >
                                    Delete
